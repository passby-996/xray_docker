...
if [ -f /config_info.txt ]; then
  ...
  if [ -z "$DEST" ]; then
    echo "DEST is not set. default value addons.mozilla.org:443"
    DEST="addons.mozilla.org:443"
  fi

  if [ -z "$SERVERNAMES" ]; then
    echo "SERVERNAMES is not set. use default value [\"addons.mozilla.org\"]"
...
echo "Public key:  $PUBLICKEY"
  ...
  if [ "$IPV4" != "null" ]; then
    SUB_IPV4="vless://$UUID@$IPV4:$EXTERNAL_PORT?encryption=none&security=reality&type=$NETWORK&sni=$FIRST_SERVERNAME&fp=firefox&pbk=$PUBLICKEY&path=$XHTTP_PATH&mode=auto#${IPV4}-freeman_docker_xhttp_reality"
    echo "IPV4 订阅连接: $SUB_IPV4" >>/config_info.txt
    echo -e "IPV4 订阅二维码:\n$(echo "$SUB_IPV4" | qrencode -o - -t UTF8)" >>/config_info.txt
  fi
  if [ "$IPV6" != "null" ];then
    SUB_IPV6="vless://$UUID@[${IPV6}]:$EXTERNAL_PORT?encryption=none&security=reality&type=$NETWORK&sni=$FIRST_SERVERNAME&fp=firefox&pbk=$PUBLICKEY&path=$XHTTP_PATH&mode=auto#${IPV6}-freeman_docker_xhttp_reality"
